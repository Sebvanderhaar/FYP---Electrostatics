...
def BindBuffers(charges, forces, positions):
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER, forces.nbytes, forces, GL_DYNAMIC_COPY)  # Initialize with forces data
...
def getResults(program, forces, charges, positions, forcesBuffer):
    ...
    # Dispatch the compute shader (execute the shader with len(charges) and len(positions) work items)
    glDispatchCompute(len(charges), len(positions), 1)
    glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT)

    # Now retrieve the forces data from the GPU
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, forcesBuffer)
...
# Initialize OpenGL and compile shader
...
# Define forces array (initialized to zero)
...
# Run compute shader and get results
forces = getResults(program, forces, charges, posArray, forcesBuffer)

# Convert the forces to a NumPy array and reshape them
results = np.frombuffer(forces, dtype=np.float32).reshape(6, 3, 2)

print("Results from compute shader (first 6 vec2s):\n", results[:6])

# Cleanup
cleanUp(program, shader, chargesBuffer, forcesBuffer, positionsBuffer)
