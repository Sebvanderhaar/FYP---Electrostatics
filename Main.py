...
def DrawFieldLine(surface: pygame.surface, charge: np.ndarray, i: int, totLines: int, chargeArray: np.ndarray) -> None:
    ...
    r: float = 5
    colour: tuple = (200,200,200)
    initOffset: np.ndarray = np.array([r*np.cos(i*2*np.pi/totLines), r*np.sin(i*2*np.pi/totLines)])
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:
                for chargeIndex, charge in enumerate(chargeArray):
                    if np.linalg.norm(np.array(pygame.mouse.get_pos()) - charge[1]) < 10:
                        dragging = True
                        dragger = chargeIndex
